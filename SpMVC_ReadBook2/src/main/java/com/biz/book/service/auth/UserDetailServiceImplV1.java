 ...
 * UserDetailService 를 Customizing 커스터마이징
 * 
 * Customizing
 * 패키지형 솔루션을 가지고 있는 IT 회사에서
 * 어떤 회사에 솔루션을 판매하면서
 * 회사의 실정, 업무환경, 여러가지 여건들을 요구분석 하여
 * 솔루션을 사용하는 회사에 최적화 하는것
 */
...
public class UserDetailServiceImplV1 implements UserDetailsService{
	 ...
	 * @Autowired로 주입 받은 객체에 메모리 누수현상이 발생을 하더라
	 * 
	 ...
	 * 3. 주입받을 객체의 개수가 늘거나 줄면 생성자를 또다시 변경해야 하는 번거로움이있다
	 * 
	 * lombok의 @RequiredArgsConstructor를 사용하면
	 * final로 선언된 모든 필드변수들을 모아서 생성자로 만들어 준다
	 * 
	 */
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		*/
		...
		/*
		 * 사용자 정보 테이블에서 enable칼럼의 값이 boolean형으로
		 * 설정을하고 푀초에 회원가입을 했을때 이 값을 false로 세팅을 하고 email 인증과 같은절차를 통과 했을때 이 값을 true로 만들어서
		 * 로그인 이 될수 있도록 설정
		 */
