 ...
 * 
 * RequestMapping method에 @ModelAttrubute("객체이름") 클래스 객체 형식으로
...
public class MemberController {
    ...
    * 
    * @ModelAttribute("이름")을 설정했을 경우 1. form에서 POST로 데이터를 보냇을 경우 form에서 보낸 데이터가 담긴
    * VO 객체를 생성하여 method내의 코드에서 사용할수 있도록 준비해준다. 2. 아무도(아무곳에서도) 매개변수와 클래스와 일치하는 변수를
    * 전달하지 않을경우 자체적으로 클래스의 생성자를 호출하여 비어있는 객체를 만들어서 method내에 코드에서 사용할수 있도록 준비해 준다.
    */
   ...
   public String join(@ModelAttribute("memberVO") UserDetailsVO userVO, Model model) {
      ...
      // 이코드를 @ModelAttribute("memberVO")가 대신한다.
    ...
    * member-write2.jsp 열고 나머지 데이터를 입력 한 후
    * join-comp POST로 보내면
    * 먼저 입력받아서 SessionAttributes에 보관중인 id, 비번과
    * 나중에 입력한 이름, 전화번로 등등 과 함께 묶어서
   ...
   public String id_check(String username) {
      ...
      // 문법 오류가 발생을하면 구체적으로 memberService method를 구현하는 방법
