 ...
 *  이때 spring security는 authentication객체에 로그인한
 ...
 *  user(member) table에서 select하여 teturn한다
...
public class AuthProviderImpl implements AuthenticationProvider {
   ...
   public Authentication authenticate(Authentication authentication) throws AuthenticationException {
	...
	// PasswordEncoder로 암호화된 비번 비교
      /*
       * 사용자가 입력한 password 평문 문자열을 내부에서 암호화 하여 DB에 저장되어 있는 암호화 비번
       */
     if (!passwordEncoder.matches(password,userVO.getPassword())) {
      ...
      //3. 유효한 사용자 정보인가
      if(!userVO.isEnabled()) {
    	  throw new BadCredentialsException(username + "사용자정보 사용불가!!");
      }
      ...
      // 사용자의  detail정보와 roll정보를 token에 닽이 심어 놓는다
