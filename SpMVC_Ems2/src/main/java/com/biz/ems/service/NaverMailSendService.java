...
public class NaverMailSendService {
	...
	public void sendMail(EmsVO emsVO) {
		 ...
		 * 이문서는 Web Browser를 통해서 내용을 확인 할수 있다
		 * 이처럼 Mime type을 지정해둔 문서는 인터넷으로부터 받았을때
		 * 즉시 내용을 확인할수 있다
		 * 
		 * xMail을 통해서 지금만드는 Mime type은 email viewer에서
		 * 확인할수 있는 문서 형식이다
		 * 이 문서는 메모장이나, Web Browser로 보면 그내용을 정확히 확인이 어렵다
		 * 대쵸적으로 xMail Mime type 문서를 볼수 있는 어플리케이션에
		 * 아웃룩이 있고, 각 스마트기기의 고유 Email App이 이런종류에 해당한다
		 */
		...
		try {
			// 두번째 값을 true : HTML 적용을 한메세지를 보낼수 있다
			// 두번째 값을 true : 메세지에 이미지 파일을 적용하여 보낼수 있다
			...
			// 두번째값을 true : HTML 적용을 한 메세지를 보낼수 있다
