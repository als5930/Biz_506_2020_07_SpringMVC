...
public class ProductController {
	 ...
	 * URL에 담긴 변수 이름과 실제 사용하는 파라메터 변수가 다를때는
	 * @RequestParam()을 사용하는 방법이 있다
	 * @RequestPatam()은 spring 4.x 이상에서는 선택사항이다
	 * "query_이름" 파라메터 변수이름이 같을때는 생략해도 된다
	 * 단, 이 Annotation은 primitive형 이거나
	 * wrapper(Interger, Chanractor, String)일 경우만 사용가능하다
	 * 
	...
	public String detail(@RequestParam("id") String p_code,Model model) {
		...
		ProductVO proVO= proService.finById(p_code);
	 ...
	 * 다시한번 /product/URL로 request를 수행해 달라는 요청
	 * 상태코드 304번
	 */
	...
	public String update(@RequestParam("id") String p_code, @ModelAttribute("PRO_VO") ProductVO proVO, Model model) {
		...
		proVO = proService.finById(p_code);
		model.addAttribute("PRO_VO",proVO);
		model.addAttribute("BODY","PRO_WRITE");
