...
<%
	/*
spring security에서 ROLE 저보를 확인하여
...
<script>
	...
	// 최근의JS에서는 별로 사용을 권하지 않은 event 이다
...
<nav id="main-nav">
	<ul>
		...
		<%
			/*
			isAnonymous()
			현재 화면에서 로그인한 세션 정보를 찾을수 없으면 회원가입, 로그인 메뉴를 보여라
			*/
		%>
		...
		<%
			/*
			hesRole('ROLE_ADMIN)
			로그인 세션정보가 있고ROLE(권한)정보가 admin이면
			*/
		%>
		...
		<%
			/*
			hesRole('ROLE_USER)
			로그인 세션정보가 있고ROLE(권한)정보가 USER이면
			*/
		%>
		...
		<%
			/*
			hesRole('ROLE_GUEST)
			로그인 세션정보가 있고ROLE(권한)정보가 GUEST이면
			*/
		%>
