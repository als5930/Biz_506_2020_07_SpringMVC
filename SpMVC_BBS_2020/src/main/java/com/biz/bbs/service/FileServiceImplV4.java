...
/*
 * 파일을 upload 프로젝트에서 외부에 파일을 공개하기위해 폴더를 개방해 두면
 * 폴더를 외부에서 접근하여 해킹을 수행하는 경우들이 발생할수 있다
 * 
 * 특히, 프로젝트 구조를 알게되는 경우 같은이름의 파일을 엉뚱한 파일로
...
public class FileServiceImplV4 extends FileServiceImplV1{
	...
	/*
	 * 필드(맴버)변술를 private final로 선언했을 경우
	 * 보통 final로 선언된 변수는 선언돠 동시에 생성(초기화)를 해야한다
	 * private final로 선언된 맴버 변수는
	 * 클래스의 생성자 매서드에서 초기화하는 것을 허용한다
	 * 
	 * private final로 선언된 맴버변수는 반드시 클래스의 생성자 매서드에서 초기화를 해야한다
	 */
	...
	public String fileUp(MultipartFile file) {
		 ...
		 * 1. UUID값을 생성하고
		 * 2. 원본파일 이름에 UUID를 부착하기
		 * 3. UUID값이 부착된 파일이름은 서버에 업로드가 될것이고
		 * 만약 해커가 해당파일이름을 알게되어 동일한 이름의 파일을 만들어
		 * 업로드하면, 다시 새로운 UUID가 부착되어 원래 저장된 파일을 보호 한다
		 */
