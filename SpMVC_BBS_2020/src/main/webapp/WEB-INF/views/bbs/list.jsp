...
<script>
   document.addEventListener("DOMContentLoaded", function() {
      document.querySelector("#bbs-write")
            ...
            /*
            tag와 tag들이 서로 감싼관계로 layout이 만들어져 있을때
            tag들에 click event 핸들링이 설정되어 있으면
            어떤 특정 tag를 click 했을때 원하지 않는 event가 발생하는 경우가 있다
            이러한 현상을 이벤트 버블링 이라고 한다
            tag box의 가장 중간부분에 있는 tag를 클릭하면 안쪽부터 바깥쪽으로 계속해서 이벤트가 전해지는 현상
            이러한 현상을 막기위해서 event.stopPropagation() 등의 함수를 사용하여
            버블링을 막아준다
            
            이 버블링을 역으로 효과적으로 이용할수 있는데
            td를 감싸고 있는 table에 click event를 설정하고
            td레서는 click event를 제거해주면
            
            결국 td를 click 했을때 해야할 일을 table을 click event에서
            통합적으로 관리를 할수 있다
            복잡한 event코드 같으면 td에서 처리하는 것이 효과적으일 수도 잇지만
            짧은 코드를 반복적하지 않고 처리하기 위해서 table에서 처리한다
